"""
Multi-Objective EWMA Optimization (LITE VERSION)
=================================================
Find parameters that beat baseline on BOTH DA and QA

Optimized for memory:
- Smaller search space
- Pre-compute model scores once
- Process one horizon at a time
- Garbage collection between horizons
"""
import pandas as pd
import numpy as np
import os
...
L_VALUES = [4, 6, 8]           # Reduced from 6 values
ALPHA_VALUES = [0.2, 0.3, 0.4]  # Reduced from 5 values
N_VALUES = [5, 7, 10]           # Reduced from 4 values
DA_WEIGHTS = [0.5, 0.6, 0.7]    # Reduced from 4 values

total_combos = len(L_VALUES) * len(ALPHA_VALUES) * len(N_VALUES) * len(DA_WEIGHTS)
print(f"    Combos per horizon: {total_combos}")
...
def precompute_model_scores(df_horizon):
    """Pre-compute DA and QA scores for all models ONCE"""
    train_data = df_horizon[df_horizon['time'] <= TRAIN_END]
    n_predict = int(df_horizon['n_predict'].iloc[0])
    ...
    for symbol in df_horizon['symbol'].unique():
        ...
        # Simplified QA calculation (avoid date loop)
        # Compare predicted price to current price as proxy
        pred_prices = train_model['close_predict'].values
        current_prices = train_model['target_var_price'].values
        # Use percentage error as QA proxy
        pct_errors = np.abs(pred_prices - current_prices) / np.maximum(current_prices, 1)
        train_qa = max(0, 1 - np.mean(pct_errors))
        
        model_scores[symbol] = {'da': train_da, 'qa': train_qa}
    
    return model_scores

def get_top_models(model_scores, n=5, da_weight=0.7):
    """Get top N models using pre-computed scores"""
    scored = []
    for symbol, scores in model_scores.items():
        combined = da_weight * scores['da'] + (1 - da_weight) * scores['qa']
        scored.append((symbol, combined))
...
print("\n[3] Running grid search (one horizon at a time)...")
...
for horizon in HORIZONS:
    ...
    # Load only this horizon's data
    df_h = df[df['n_predict'] == horizon].copy()
    baseline_da = baselines[horizon]['da']
    baseline_qa = baselines[horizon]['qa']
    
    # Pre-compute model scores ONCE for this horizon
    print("    Pre-computing model scores...")
    model_scores = precompute_model_scores(df_h)
    print(f"    {len(model_scores)} models scored")
            ...
            for N in N_VALUES:
                for da_weight in DA_WEIGHTS:
                    ...
                    if combo_count % 20 == 0:
                        ...
                        print(f"    Combo {combo_count}/{total_combos} ({elapsed:.0f}s)")
                    
                    # Use pre-computed scores
                    top_models = get_top_models(model_scores, n=N, da_weight=da_weight)
    ...
    if winners:
        ...
        print(f"\n  ✅ FOUND {len(winners)} winning combos!")
        print(f"     Best: L={best['L']}, α={best['alpha']}, N={best['N']}, w={best['da_weight']}")
        print(f"     DA: {best['da']:.2f}% (+{best['da_improvement']:.2f})")
        print(f"     QA: {best['qa']:.2f}% (+{best['qa_improvement']:.2f})")
    else:
        horizon_results.sort(key=lambda x: min(x['da_improvement'], x['qa_improvement']), reverse=True)
        if horizon_results:
            best = horizon_results[0]
            print(f"\n  ⚠️  No combo beats BOTH")
            print(f"     Closest: L={best['L']}, α={best['alpha']}, N={best['N']}")
            print(f"     DA: {best['da']:.2f}% ({best['da_improvement']:+.2f})")
            print(f"     QA: {best['qa']:.2f}% ({best['qa_improvement']:+.2f})")
    
    # Clean up memory before next horizon
    del df_h, model_scores, horizon_results
    gc.collect()
...
print("\n")
print("=" * 80)
print(f"FINAL RESULTS (completed in {elapsed_total:.0f} seconds)")
print("=" * 80)

print("\n┌────────────────────────────────────────────────────────────────────────┐")
print("│            OPTIMAL PARAMETERS (BEATS BOTH METRICS)                     │")
print("├─────────┬─────┬───────┬─────┬───────┬─────────┬─────────┬───────┬──────┤")
print("│ Horizon │  L  │   α   │  N  │  Wt   │   DA    │   QA    │ DA+   │ QA+  │")
print("├─────────┼─────┼───────┼─────┼───────┼─────────┼─────────┼───────┼──────┤")

for horizon in HORIZONS:
    if horizon in best_per_horizon:
        b = best_per_horizon[horizon]
        print(f"│  {horizon:>4}d  │ {b['L']:>3} │ {b['alpha']:>5.2f} │ {b['N']:>3} │ {b['da_weight']:>5.2f} │ {b['da']:>6.2f}% │ {b['qa']:>6.2f}% │ +{b['da_improvement']:>4.1f} │ +{b['qa_improvement']:>3.1f} │")
    else:
        print(f"│  {horizon:>4}d  │  -  │   -   │  -  │   -   │    -    │    -    │   -   │   -  │")
...
print(f"   • Beat BOTH metrics: {total_winners}")
print(f"   • Winning horizons: {len(best_per_horizon)}/{len(HORIZONS)}")

if best_per_horizon:
    avg_da = np.mean([b['da_improvement'] for b in best_per_horizon.values()])
    avg_qa = np.mean([b['qa_improvement'] for b in best_per_horizon.values()])
    print(f"   • Avg DA improvement: +{avg_da:.2f} pp")
